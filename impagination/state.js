...
export default class State {
  ...
  get hasUnrequested() {
    return this.pages.some((p) => !p.isRequested);
  }
  get hasRequested() {
    return this.pages.some((p) => p.isRequested);
  }
  get hasPending() {
    return this.pages.some((p) => p.isPending);
  }
  get hasResolved() {
    return this.pages.some((p) => p.isResolved);
  }
  get hasRejected() {
    return this.pages.some((p) => p.isRejected);
  }
  get hasUnfetchable() {
  ...
  // fetchable
  get unrequested() {
    return this.pages.filter((p) => !p.isRequested);
  }
  get requested() {
    return this.pages.filter((p) => p.isRequested);
  }
  get pending() {
    return this.pages.filter((p) => p.isPending);
  }
  get resolved() {
    return this.pages.filter((p) => p.isResolved);
  }
  get rejected() {
    return this.pages.filter((p) => p.isRejected);
  }
  get unfetchable() {
  ...
  unfetch(unfetchable = []) {
    ...
    return new State(this, {
      _unfetchablePages: this._unfetchablePages.filter(
        (p) => !unfetchable.includes(p)
      ),
    });
  }
  ...
  splice(start, deleteCount, ...items) {
    ...
    try {
      ...
      throw Error(
        `Impagination could not find resolved page for record at index ${record.index}`
      );
    }
  ...
  _getLoadHorizons() {
    ...
    let maxLoadHorizon = Math.min(
      this.stats.totalPages || Infinity,
      maxLoadPage
    );
