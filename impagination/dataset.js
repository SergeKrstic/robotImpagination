...
export default class Dataset {
  ...
  post(data, index = 0) {
    try {
      ...
      console.error(
        `Error: Impagination did not POST ${data}. Could not find resolved page for record at index ${index}`
      );
    }
    this.observe(this.state);
  }
  ...
  put(data, index) {
    ...
    try {
      ...
      console.error(
        `Error: Impagination did not PUT ${data}. Could not find resolved page for record at index ${index}`
      );
    }
    this.observe(this.state);
  }
  ...
  delete(index) {
    ...
    try {
      ...
      console.error(
        `Error: Impagination did not DELETE record at ${index}. Could not find resolved page for record at index ${index}`
      );
    }
    this.observe(this.state);
  }
  ...
  _fetchPages(fetchable) {
    ...
    fetchable.forEach((page) => {
      return this.fetch
        .call(this, page.offset, this.state.pageSize, stats)
        .then((records = []) => {
          return this.observe(
            (this.state = this.state.resolve(records, page.offset, stats))
          );
        })
        .catch((error = {}) => {
          return this.observe(
            (this.state = this.state.reject(error, page, stats))
          );
        });
    });
